...
class ApplyJob
{
    ...
    public function addJobApplication($candidate_data){

        $my_post = array(
            'post_title'    => $candidate_data['last_name'] ? $candidate_data['first_name'] . " " . $candidate_data['last_name'] : $candidate_data['first_name'],
        ...
        $jobDivaID = get_field("job_external_id", $candidate_data['job_id']);

        $formatted_date = date("d/m/Y", time());
        $formatted_time = date("h:i A", time());

        update_field( "email_address", $candidate_data['email'], $postId );
        update_field( "jobdiva_post_id", $jobDivaID, $postId );
        update_field( "submission_date", $formatted_date, $postId );
        update_field( "submission_time", $formatted_time, $postId );
    }
    ...
    public function apply( WP_REST_Request $request )
    {
        ...
        $this->addJobApplication($candidate_data);
